...
import { useGetStockQuote, useGetStockHistory, GetStockHistoryPeriod, getGetStockQuoteQueryKey, getGetStockHistoryQueryKey } from '@workspace/api-client-react';
import { AreaChart, Area, ResponsiveContainer, YAxis } from 'recharts';

export default function Dashboard() {
  const [, setLocation] = useLocation();
  const [activePeriod, setActivePeriod] = useState<GetStockHistoryPeriod>(GetStockHistoryPeriod['1D'] as any);
  const [isLive, setIsLive] = useState(true);
  
  useEffect(() => {
    const user = JSON.parse(localStorage.getItem('spcx_user') || 'null');
    if (!user) {
      setLocation('/signin');
    }
  }, [setLocation]);
  ...
  const handlePeriodClick = (p: string) => {
    if (p === 'Live') {
      setIsLive(true);
      setActivePeriod(GetStockHistoryPeriod['1D']);
    } else {
      setIsLive(false);
      ...
      <main className="flex-1 px-6 py-8 max-w-4xl mx-auto w-full">
        ...
        >
          ...
          <div className="flex items-center gap-2 text-red-500 font-display text-lg tracking-wider">
            ...
            <span>
              ${quote?.change ? Math.abs(quote.change).toFixed(2) : "1.86"} 
              ({quote?.changePct ? Math.abs(quote.changePct).toFixed(2) : "1.24"}%)
            </span>
          ...
          {periods.map(p => {
            ...
            return (
              <button
                ...
                className={`px-4 py-1.5 rounded-full text-xs font-display font-medium tracking-widest transition-all ${
                  isActive
        ...
        <motion.div
          ...
          className="h-[200px] w-full mb-12 -mx-2"
        >
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            {[
              ...
              <motion.div 
                ...
                className="bg-[#0a0f14] p-4 border border-white/5"
              >
        ...
        >
          <div className="bg-[#0a0f14] p-6 border border-white/5 relative overflow-hidden">
            ...
            <div className="flex justify-between items-end mb-1">
              <div className="text-2xl font-bold font-display tracking-wider">0 Shares</div>
              <div className="text-right">
                <div className="text-sm text-white/50 font-display tracking-widest uppercase mb-1">Market Value</div>
                <div className="text-xl font-bold font-display tracking-wider">$0.00</div>
              </div>
            </div>
            <div className="text-sm text-white/50 font-display tracking-widest uppercase mt-4">Avg Cost: $0.00</div>
          </div>
        </motion.div>
        ...
        <div className="mt-16 mb-8 text-center">
          <button 
            ...
            className="text-white/40 hover:text-white transition-colors text-sm font-display tracking-widest uppercase underline decoration-white/20 underline-offset-4 cursor-pointer"
          >
            Sign out
          </button>
        </div>
